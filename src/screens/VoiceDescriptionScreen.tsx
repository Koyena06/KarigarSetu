    ...
    return (
      <ScreenWrapper>
        ...
        <View className="gap-3">
          <Button onPress={retry}><View className="flex-row items-center gap-2"><RotateCcw size={20} color="#FFFDF8" /><Text className="text-cream-50 font-body-bold text-lg">Retry</Text></View></Button>
  ...
  return (
    <ScreenWrapper>
      ...
      <View className="flex-1 items-center py-5">
        <View className="w-20 h-20 rounded-md bg-lavender-100 items-center justify-center mb-5"><Mic size={38} color="#60419B" /></View>
        <Text className="text-2xl font-heading-bold text-forest-700 text-center">Tell us about your product</Text>
        ...
        {status === 'processing' ? (
          <View className="w-full mt-10"><Waveform active /><Text className="text-center text-lg font-heading-bold text-forest-700 mt-5">Creating your description…</Text><Text className="text-center text-sm text-forest-400 mt-2">Processing your voice note</Text></View>
        ) : status === 'transcribed' ? (
          <View className="w-full mt-8">
            <View className="bg-mist-50 border border-mist-200 rounded-md p-5">
              <Text className="text-xs font-heading-bold uppercase tracking-wide text-mist-500">Demo transcription</Text>
              ...
              <View className="flex-row gap-2 mt-4">
                <Pressable onPress={() => void tryPlayRecording()} className="flex-1 min-h-12 rounded-md bg-cream-50 border border-cream-200 flex-row items-center justify-center gap-2"><Play size={18} color="#1B5938" /><Text className="font-body-bold text-forest-600">Play recording</Text></Pressable>
                <Pressable onPress={readAloud} className="flex-1 min-h-12 rounded-md bg-forest-500 flex-row items-center justify-center gap-2"><Volume2 size={18} color="#FFFDF8" /><Text className="font-body-bold text-cream-50">Read aloud</Text></Pressable>
              </View>
            </View>
          </View>
        ) : (
          <View className="items-center mt-8 w-full">
            <Text className="text-4xl font-heading-bold tracking-wider text-forest-700">{formatTime(seconds)}</Text>
            ...
            <Text className="text-sm font-body-semibold text-forest-400 mt-4">{status === 'recording' ? 'Tap to stop recording' : 'Tap to start recording'}</Text>
          </View>
        )}
        ...
        <View className="w-full mt-auto pt-8">
          ...
          <View className="gap-3 mt-5">
            ...
            {status !== 'processing' && <Button variant="ghost" onPress={() => navigate('enhance')}><View className="flex-row items-center gap-2"><X size={19} color="#1B5938" /><Text className="text-forest-600 font-body-bold text-lg">{t.cancel}</Text></View></Button>}
          </View>
        </View>
      </View>
    </ScreenWrapper>
